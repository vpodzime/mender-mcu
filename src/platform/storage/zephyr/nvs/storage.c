...
static void
print_depl_log_msg(const char *msg, MENDER_ARG_UNUSED void *ctx) {
    mender_log_debug("Read from depl logs FCB: %s", msg);
}
...
mender_storage_init(void) {
    ...
    depl_logs_buffer.f_magic       = 0x2AABEE35;
    ...
    mender_storage_deployment_log_walk(print_depl_log_msg, NULL);
#endif /* CONFIG_MENDER_DEPLOYMENT_LOGS */
...
#ifdef CONFIG_MENDER_DEPLOYMENT_LOGS
mender_err_t
mender_storage_deployment_log_append(const char *msg) {
    int          result;
    ...
    result                 = fcb_append(&depl_logs_buffer, msg_len + 1, &entry);
    if (-ENOSPC == result) {
        if (0 != (result = fcb_rotate(&depl_logs_buffer))) {
            mender_log_error("Failed to rotate the deployment logs FCB");
            /* if (0 != (result = fcb_append_to_scratch(&depl_logs_buffer))) { */
            /*     mender_log_error("Failed to start using the scratch sector for the deployment logs FCB"); */
            ret = MENDER_FAIL;
            goto END;
        }
    ...
    if (0 != (result = flash_area_write(fap, entry.fe_sector->fs_off + entry.fe_data_off, msg, MIN(DEPL_LOGS_MAX_MSG_LEN, msg_len + 1)))) {
        mender_log_error("Failed to write message to the FCB entry [%d]", -result);
        ret = MENDER_FAIL;
        goto END;
    }
